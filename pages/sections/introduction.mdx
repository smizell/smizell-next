...
It's not as wild as it sounds to treat YAML as code. AWS and Azure do it, why can't we?

But maybe it's changing. AWS has created [CDK](https://aws.amazon.com/cdk/) to let people write code in programming languages like Python rather than write YAML.
...
Is the industry is fine with putting code in YAML? Or is there a shift happening?
...
There are upsides to adding dynamic features to a specification. One reason is that it allows people to write logic or take advantage of reusability without installing development environments. A text editor is all you need.
