...
AWS is not shy about calling this "infrastructure as code" even though it's YAML.
...
There's also a proposal for [Overlays](https://github.com/OAI/OpenAPI-Specification/blob/master/proposals/004_Overlays.md) on the table that creates a code-like specification. People have expressed their desire for dynamic features.

## See, everyone else is doing it!

Have you ever thought about how much the software world is built on YAML? AWS, Azure, Google Cloud use it for defining infrastructure. Kubernetes uses it for orchestration. Docker Compose uses it for containers. On and on. The world runs on YAML!

It seems that AWS has decided code in a programming language would be better than YAML. They created [CDK](https://aws.amazon.com/cdk/) to let people write code that converted into the more complex YAML you saw in the example.
