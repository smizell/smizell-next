...
I created an object with two keys, `answer_to_question` and `colors` and called functions to create their values. I prefixed function calls with `fn:` and passed an array of values to the function.

Geneva makes every function of Ramda and Ramda Adjunct available, along with JSON Path Plus and another tool of my own called Saunter.

## Why would you do this to us, Stephen?

Now I'll be the first to admit that it sounds like a bad idea to put code in YAML. Developers don't want to write code in a serialization format. Non-developers don't want to learn to write code in their API definitions. It feels like a lose-lose situation.
